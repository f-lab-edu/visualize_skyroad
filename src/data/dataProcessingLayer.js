import { getDepartureAirport } from '../api/flight';
...
const getAircraftTracks = async (flight) => {
...
const requestFlightTrack = async (flight) => {
...
const requestFlightList = async ({ departureAirport, arrivalAirport, }) => {
    ...
    return await getDepartureAirport(departureAirport['icao']).then((flights) => {
        const flightList = flights.filter((flight) => flight.estArrivalAirport === arrivalAirport['icao']);
        // .map((flight: Flight) => ({
        //     ...flight,
