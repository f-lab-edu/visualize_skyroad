...
describe('FlightOnMap', () => {
    ...
    it('항공편 데이터가 없을 때 에러 메시지를 표시해야 합니다', () => {
        render(React.createElement(MemoryRouter, null,
            React.createElement(FlightOnMap, null)));
        expect(screen.getByText('항공상세정보를 가져오지 못하였습니다.')).toBeInTheDocument();
    });
    it('항공편 데이터가 있을 때 지도를 표시해야 합니다', async () => {
        render(React.createElement(MemoryRouter, { initialEntries: [{ state: mockFlightData }] },
            React.createElement(FlightOnMap, null)));
        await waitFor(() => {
