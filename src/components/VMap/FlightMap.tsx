...
const FlightMap: React.FC = ({}) => {
  ...
  const [line, setLine] =
    useState<FeatureCollection>(/* todo: explict empty obj */)
  ...
  const getLineFromRoute = async (): Promise<FeatureCollection> => {
    return new Promise(async (resolve) => {
      const { departure, arrival }: { departure: Airport; arrival: Airport } =
        location.state || null
  ...
  const { play, pause, stop, isPlaying, isPaused } = useAnimationController(
    handleUpdate,
    handleStart,
    handleStop,
    totalFrames
