...
const useMapAnimationController = ({ map, line, zoomLevel, }) => {
    const [isPlaying, setIsPlaying] = useState(false);
    const [isPaused, setIsPaused] = useState(false);
    const [mergedLine, setMergedLine] = useState(null);
    const [currentFrame, setCurrentFrame] = useState(0);
    const requestRef = useRef(null);
    const previousTimeRef = useRef(null);
    const { bearing, addBearing, calculateBearing } = useBearingWithMovingAverage(BEARING_LIST_LENGTH);
    const calculateSpeed = (coordinates) => {
    ...
    const handleUpdate = (deltaTime) => {
        setCurrentFrame((prevFrame) => {
            if (!mergedLine?.features[0]?.geometry.coordinates)
                return prevFrame;
            ...
            if (coordinates[nextFrame]) {
                ...
                const calculatedBearing = calculateBearing(currentPosition[1], currentPosition[0], nextPosition[1], nextPosition[0]);
    ...
    const mergeFeatureCollectionCoordinates = (featureA, featureB) => {
        if (featureB === null)
            return featureA;
