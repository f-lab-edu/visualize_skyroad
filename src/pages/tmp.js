...
const FlightMap = ({ flightData, }) => {
    ...
    const { totalDistance, segmentDistances } = useMemo(() => {
        ...
        for (let i = 0; i < flightData.path.length - 1; i++) {
            const distance = calculateDistance(flightData.path[i].latitude, flightData.path[i].longitude, flightData.path[i + 1].latitude, flightData.path[i + 1].longitude);
    ...
    // 애니메이션 프레임 계산
    const calculateFrame = useCallback((elapsed) => {
        ...
        // 현재 세그먼트 찾기
        while (distanceCovered > segmentDistances[currentSegment] &&
    ...
    return (React.createElement("div", { className: "relative w-full h-screen" },
        React.createElement("div", { className: "absolute top-4 left-4 z-10 bg-white p-4 rounded-lg shadow-lg" },
            React.createElement("div", { className: "space-y-4" },
                React.createElement("div", { className: "flex items-center gap-4" },
                    React.createElement("button", { className: "p-2 rounded bg-blue-500 text-white hover:bg-blue-600", onClick: handlePlayPause }, controls.isPlaying ? React.createElement(Pause, { size: 20 }) : React.createElement(Play, { size: 20 })),
                    React.createElement("button", { className: "p-2 rounded bg-gray-500 text-white hover:bg-gray-600", onClick: handleReset },
                ...
                React.createElement("div", null,
                    React.createElement("label", { className: "block text-sm font-medium text-gray-700" },
                        ...
                        " km/h",
                        React.createElement("input", { className: "w-full mt-1", max: "1000", min: "100", onChange: (e) => setControls((prev) => ({
                                ...prev,
                                currentSpeed: Number(e.target.value),
                            })), step: "50", type: "range", value: controls.currentSpeed }))),
                React.createElement("div", { className: "space-y-2" },
                    React.createElement("div", { className: "h-2 bg-gray-200 rounded" },
                        React.createElement("div", { className: "h-full bg-blue-500 rounded", style: { width: `${controls.progress * 100}%` } })),
                    React.createElement("div", { className: "text-sm text-gray-600" },
                        "Remaining: ",
                        Math.round(controls.remainingDistance),
                        " km"),
                    React.createElement("div", { className: "text-sm text-gray-600" },
                        "Time: ",
        ...
        React.createElement(Map, { initialViewState: {
                ...
                zoom: 6,
            }, mapLib: maplibregl, mapStyle: `https://api.maptiler.com/maps/basic-v2/style.json?key=${import.meta.env.VITE_MAPTILER_KEY}`, ref: mapRef, style: { width: '100%', height: '100%' } },
            React.createElement(Source, { data: {
                    type: 'Feature',
                    geometry: {
                        type: 'LineString',
                    ...
                    properties: {},
                }, id: "flight-path", type: "geojson" },
                React.createElement(Layer, { id: "flight-path-layer", paint: {
                        'line-color': '#3b82f6',
                        'line-width': 3,
                        'line-opacity': 0.8,
                    }, type: "line" })),
            React.createElement(Marker, { anchor: "center", latitude: position[1], longitude: position[0] },
                React.createElement("div", { className: "w-0 h-0 border-l-[10px] border-r-[10px] border-b-[20px] border-l-transparent border-r-transparent border-b-red-500", style: {
